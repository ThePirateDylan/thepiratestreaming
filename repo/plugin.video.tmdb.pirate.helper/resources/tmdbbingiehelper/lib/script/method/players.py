...
from tmdbbingiehelper.lib.script.method.decorators import get_tmdb_id, map_kwargs
...
def play_external(**kwargs):
    ...
    from tmdbbingiehelper.lib.player.players import Players
    kodi_log(['lib.script.router - attempting to play\n', kwargs], 1)
    Players(**kwargs).play()


def play_using(play_using, mode='play', **kwargs):
    from tmdbbingiehelper.lib.addon.plugin import get_infolabel
    from tmdbbingiehelper.lib.files.futils import read_file
    from bingie.parser import parse_paramstring
    ...
    def _update_from_listitem(dictionary):
        ...
        if url[-5:] == '.strm':
            url = read_file(url)
        params = {}
        if url.startswith('plugin://plugin.video.tmdb.pirate.helper/?'):
            params = parse_paramstring(url.replace('plugin://plugin.video.tmdb.pirate.helper/?', ''))
...
def update_players():
    ...
    from tmdbbingiehelper.lib.files.downloader import Downloader
    from tmdbbingiehelper.lib.addon.plugin import set_setting
    ...
    downloader = Downloader(
        extract_to='special://profile/addon_data/plugin.video.tmdb.pirate.helper/players',
...
def set_chosenplayer(tmdb_type, tmdb_id, season=None, episode=None, **kwargs):
    ...
    from xbmcgui import Dialog
    from tmdbbingiehelper.lib.player.players import Players
    from tmdbbingiehelper.lib.addon.consts import PLAYERS_CHOSEN_DEFAULTS_FILENAME
    from tmdbbingiehelper.lib.files.futils import get_json_filecache, set_json_filecache
    from tmdbbingiehelper.lib.addon.plugin import get_localized
