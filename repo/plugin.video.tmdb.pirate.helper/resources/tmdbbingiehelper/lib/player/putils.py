...
def make_upnext(current_episode, next_episode):
    import AddonSignals
    from tmdbbingiehelper.lib.addon.consts import UPNEXT_EPISODE
    next_info = {
    ...
    AddonSignals.sendSignal('upnext_data', next_info, source_id='plugin.video.tmdb.pirate.helper')


def get_players_from_file():
    from json import loads
    from tmdbbingiehelper.lib.files.futils import get_files_in_folder, read_file
    from tmdbbingiehelper.lib.addon.plugin import get_setting, get_condvisibility
    from tmdbbingiehelper.lib.addon.consts import PLAYERS_BASEDIR_BUNDLED, PLAYERS_BASEDIR_USER, PLAYERS_BASEDIR_SAVE, PLAYERS_REQUIRED_IDS
