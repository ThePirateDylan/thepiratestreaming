...
from tmdbbingiehelper.lib.addon.plugin import ADDONPATH, PLUGINPATH, format_folderpath, get_localized, get_setting, executebuiltin, get_infolabel
from bingie.parser import try_int, try_float, boolean
from tmdbbingiehelper.lib.addon.consts import PLAYERS_PRIORITY, PLAYERS_CHOSEN_DEFAULTS_FILENAME
from tmdbbingiehelper.lib.items.listitem import ListItem
from tmdbbingiehelper.lib.api.kodi.rpc import get_directory, KodiLibrary
from tmdbbingiehelper.lib.player.inputter import KeyboardInputter
...
class PlayerMethods():
    ...
    def get_local_file(file):
        ...
        if file.endswith('.strm'):
            from tmdbbingiehelper.lib.files.futils import read_file
            contents = read_file(file)
            if contents.startswith('plugin://plugin.video.tmdb.pirate.helper'):
                return
            return contents
        return file
    ...
    def get_chosen_default(self):
        ...
        """
        from tmdbbingiehelper.lib.files.futils import get_json_filecache
...
class PlayerDetails():
    def get_external_ids(self):
        from tmdbbingiehelper.lib.player.details import get_external_ids
    ...
    def get_item_details(self, language=None):
        from tmdbbingiehelper.lib.player.details import get_item_details
    ...
    def set_detailed_item(self):
        from tmdbbingiehelper.lib.player.details import set_detailed_item
    ...
    def get_language_details(self, language=None, year=None):
        from tmdbbingiehelper.lib.player.details import get_language_details
    ...
    def get_playerstring(self):
        from tmdbbingiehelper.lib.player.details import get_playerstring
...
class PlayerProperties():
    ...
    def players(self):
        try:
            ...
            from tmdbbingiehelper.lib.player.putils import get_players_from_file
    ...
    def p_dialog(self):
        try:
            ...
            from tmdbbingiehelper.lib.addon.dialog import ProgressDialog
            self._p_dialog = ProgressDialog('TMDbBingieHelper', f'{get_localized(32374)}...', total=3)
            return self._p_dialog
...
class Players(PlayerProperties, PlayerDetails, PlayerMethods, PlayerHacks):
    ...
    def select_player(self, detailed=True, clear_player=False, header=get_localized(32042), combined=False):
        ...
        dialog_players = [] if not clear_player else [{
            ...
            'plugin_name': 'plugin.video.tmdb.pirate.helper',
            'plugin_icon': f'{ADDONPATH}/resources/icons/other/kodi.png'}]
    ...
    def _player_dialog_select(self, folder, auto=False):
        from tmdbbingiehelper.lib.files.futils import normalise_filesize
        d_items = []
        for f in folder:
    ...
    def queue_next_episodes(self, route='make_upnext'):
        ...
        if route == 'make_upnext':
            from tmdbbingiehelper.lib.player.putils import make_upnext
            return make_upnext(self.next_episodes[0], self.next_episodes[1])
        if route == 'make_playlist':
            from tmdbbingiehelper.lib.player.putils import make_playlist
