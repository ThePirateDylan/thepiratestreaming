from tmdbbingiehelper.lib.addon.plugin import get_infolabel, get_condvisibility
from tmdbbingiehelper.lib.addon.tmdate import convert_timestamp, get_region_date
from tmdbbingiehelper.lib.addon.logger import kodi_try_except, kodi_log
from tmdbbingiehelper.lib.files.futils import validate_join
from tmdbbingiehelper.lib.api.kodi.rpc import get_person_stats
from tmdbbingiehelper.lib.api.contains import CommonContainerAPIs
from bingie.window import WindowPropertySetter
...
class CommonMonitorDetails(CommonContainerAPIs):
    ...
    def ib(self):
        try:
            ...
            from tmdbbingiehelper.lib.items.builder import ItemBuilderService
            self._ib = ItemBuilderService(tmdb_api=self.tmdb_api, ftv_api=self.ftv_api, trakt_api=self.trakt_api)
            self._ib.ftv_api = self.ftv_api if get_setting('fanarttv_lookup') else None
            return self._ib
    ...
    def get_awards_data(self):
        try:
            filepath = validate_join('special://home/addons/plugin.video.tmdb.pirate.helper/resources/jsondata/', 'awards.json')
...
class CommonMonitorFunctions(WindowPropertySetter, CommonMonitorDetails):
    ...
    def set_iter_properties(self, dictionary: dict, keys: set, property_object=None):
        ...
        TMDbBingieHelper.ListItem.{key} = dictionary[key]
        """
    ...
    def set_indexed_properties(self, dictionary):
        ...
        if get_condvisibility("!Skin.HasSetting(TMDbBingieHelper.DisableExtendedProperties) | !String.IsEmpty(Window.Property(TMDbBingieHelper.EnableExtendedProperties))"):
    ...
    def set_date_properties(self, premiered):
        ...
        self.set_property('Premiered_Custom', date_obj.strftime(get_infolabel('Skin.String(TMDbBingieHelper.Date.Format)') or '%d %b %Y'))
