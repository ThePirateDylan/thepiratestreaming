...
from bingie.thread import ParallelThread
from bingie.dialog import BusyDialog
from bingie.reqapi import RequestAPI
from bingie.plugin import KodiPlugin

KODIPLUGIN = KodiPlugin('plugin.video.tmdb.pirate.helper')
get_localized = KODIPLUGIN.get_localized

ADDONDATA = 'special://profile/addon_data/plugin.video.tmdb.pirate.helper/'
...
class WikipediaAPI(RequestAPI):
    def __init__(self, language=None):
        ...
        self._wiki_tag_link = get_infolabel('Skin.String(TMDbBingieHelper.Wikipedia.Format.Link)') or WIKI_TAG_LINK
        self._wiki_tag_bold = get_infolabel('Skin.String(TMDbBingieHelper.Wikipedia.Format.Bold)') or WIKI_TAG_BOLD
        self._wiki_tag_emph = get_infolabel('Skin.String(TMDbBingieHelper.Wikipedia.Format.Emphasis)') or WIKI_TAG_EMPH
        self._wiki_tag_sups = get_infolabel('Skin.String(TMDbBingieHelper.Wikipedia.Format.Superscript)') or WIKI_TAG_SUPS
    ...
    def get_match(self, query, tmdb_type=None, match=''):
        ...
        if not items:
            xbmcgui.Dialog().ok(get_localized(32537), get_localized(32538).format(f'{query}'))
            return
...
class WikipediaGUI(xbmcgui.WindowXMLDialog):
    ...
    def do_init(self):
        xbmcgui.Window(10000).clearProperty('TMDbBingieHelper.Wikipedia.Backdrop')
        ...
        if self._backdrop:
            xbmcgui.Window(10000).setProperty('TMDbBingieHelper.Wikipedia.Backdrop', self._backdrop)
...
def do_wikipedia_gui(wikipedia, tmdb_type=None, **kwargs):
    ...
    ui = WikipediaGUI(
        'script-tmdbbingiehelper-wikipedia.xml', ADDONPATH, 'default', '1080i',
