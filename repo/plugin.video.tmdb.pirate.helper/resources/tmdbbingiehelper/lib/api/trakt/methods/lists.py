from tmdbbingiehelper.lib.files.bcache import use_simple_cache
from tmdbbingiehelper.lib.addon.consts import CACHE_SHORT
...
def get_simple_list(self, *args, trakt_type=None, **kwargs):
    ...
    if not response:
        return
    from tmdbbingiehelper.lib.api.trakt.items import TraktItems
...
def get_basic_list(
    ...
    if randomise:
        ...
        from tmdbbingiehelper.lib.items.pages import PaginatedItems
    ...
    from tmdbbingiehelper.lib.items.pages import get_next_page
...
def get_list_of_genres(self, trakt_type):
    ...
    from tmdbbingiehelper.lib.addon.plugin import get_setting, ADDONPATH
...
def get_list_of_lists(self, path, page: int = 1, limit: int = 250, authorize=False, next_page=True, sort_likes=False, **kwargs):
    ...
    for i in sorted_list:
        ...
        # Owner of list so set param to allow deleting later
            ...
            item['context_menu'] += [(
                get_localized(118), u'Runscript(plugin.video.tmdb.pirate.helper,{})'.format(
                    u'rename_list={list_slug}'.format(**item['params'])))]
            item['context_menu'] += [(
                get_localized(117), u'Runscript(plugin.video.tmdb.pirate.helper,{})'.format(
