...
import bingie.window as window
from bingie.parser import try_int
from tmdbbingiehelper.lib.addon.plugin import get_condvisibility, get_localized, executebuiltin
...
def get_listitem(path):
    try:
        ...
        if base == 'plugin://plugin.video.tmdb.pirate.helper/':
        ...
        if base == 'plugin://script.skinvariables/':
            from bingie.modimp import importmodule
...
class WindowManager(_EventLoop):
    def __init__(self, **kwargs):
        ...
        self._on_change_func = self._on_change_direct if get_condvisibility("Skin.HasSetting(TMDbBingieHelper.DirectCallAuto)") else self._on_change_manual
    ...
    def add_origin(self, tmdb_type, tmdb_id):
        if not tmdb_type or not tmdb_id:
            return
        path = f'plugin://plugin.video.tmdb.pirate.helper/?info=details&tmdb_type={tmdb_type}&tmdb_id={tmdb_id}'
    ...
    def make_query(self, query, tmdb_type, separator=' / '):
        ...
        if not tmdb_id:
            Dialog().notification('TMDbBingieHelper', get_localized(32310).format(query))
            return
        return f'plugin://plugin.video.tmdb.pirate.helper/?info=details&tmdb_type={tmdb_type}&tmdb_id={tmdb_id}'
