...
from tmdbbingiehelper.lib.addon.thread import ParallelThread, SafeThread
from tmdbbingiehelper.lib.addon.plugin import get_infolabel, executebuiltin, get_condvisibility, ADDONPATH
from tmdbbingiehelper.lib.api.tmdb.api import TMDb
from bingie.window import get_property, WindowProperty, wait_until_active
from bingie.parser import parse_paramstring, reconfigure_legacy_params
...
"""
Runscript(plugin.video.tmdb.pirate.helper,recommendations=)
recommendations=list_id(int)|paramstring(str)|related(bool)|action(str)|close(str) [Separate multiples with || ]
    ...
    * paramstring: the tmdbbingiehelper base path such as info=cast
...
setproperty=property(str)
    * Sets Window(Home).Property(TMDbBingieHelper.{property}) to True oninfo until infodialog closes
tmdb_type=type(str)
...
script-tmdbbingiehelper-recommendations.xml
...
class WindowRecommendationsManager():
    ...
    def open_recommendations(self):
        with BusyDialog():
            ...
            self._gui = WindowRecommendations(
                'script-tmdbbingiehelper-recommendations.xml', ADDONPATH, 'default', '1080i',
    ...
    def on_join(self, t, path):
        ...
        if data:
            from tmdbbingiehelper.lib.files.futils import json_loads as loads
            data = loads(data)
