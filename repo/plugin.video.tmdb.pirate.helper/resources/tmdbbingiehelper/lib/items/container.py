...
from bingie.parser import try_int, boolean
from tmdbbingiehelper.lib.addon.consts import NO_UNAIRED_LABEL, NO_UNAIRED_CHECK, REMOVE_EPISODE_COUNT
from tmdbbingiehelper.lib.addon.plugin import get_setting, executebuiltin, get_localized, get_condvisibility
from tmdbbingiehelper.lib.api.contains import CommonContainerAPIs
from tmdbbingiehelper.lib.addon.logger import TimerList


""" Lazyimports
from tmdbbingiehelper.lib.items.kodi import KodiDb
"""
...
class ContainerDirectoryCommon(CommonContainerAPIs):
    def __init__(self, handle, paramstring, **kwargs):
        ...
        self.paramstring = paramstring  # plugin://plugin.video.tmdb.pirate.helper?paramstring
    ...
    def context_additions(self):
        if self.context_additions_make_node:
            return [(get_localized(32496), 'RunScript(plugin.video.tmdb.pirate.helper,make_node)')]
        return []
    ...
    def is_excluded(self):
        from tmdbbingiehelper.lib.items.filters import is_excluded
        return is_excluded
    ...
    def ftv_is_cache_only(self):
        ...
        if self.is_fanarttv == 'false':
            return True
        if not get_setting('fanarttv_lookup'):  # user settings
            return False
        return False
    ...
    def get_directory(self, items_only=False, build_items=True):
        ...
        if self.log_timers:
            from tmdbbingiehelper.lib.addon.logger import log_timer_report
...
class ContainerDirectoryItemDetails(ContainerDirectoryCommon):
    @cached_property
    def lidc(self):
        from tmdbbingiehelper.lib.items.database.listitem import ListItemDetailsConfigurator
