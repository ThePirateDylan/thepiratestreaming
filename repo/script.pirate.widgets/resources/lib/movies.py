...
class Movies(object):
    ...
    def listing(self):
        ...
        all_items = [
            (label_prefix + self.addon.getLocalizedString(32014), "inprogress&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32019), "recent&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32001), "recommended&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32015),
             "inprogressandrecommended&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32026), "random&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32033), "unwatched&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + self.addon.getLocalizedString(32047), "mylist&mediatype=movies&tag=%s" % tag, icon),
            (label_prefix + xbmc.getLocalizedString(135),
             "browsegenres&mediatype=movies&tag=%s" % tag, icon)
        ]
        if not tag:
            all_items += [
                (self.addon.getLocalizedString(32004), "similar&mediatype=movies&tag=%s" % tag, icon),
                (self.addon.getLocalizedString(32043), "playlistslisting&mediatype=movies", icon),
                (self.addon.getLocalizedString(32041), "playlistslisting&mediatype=movies&tag=ref", icon),
    ...
    def tagslisting(self):
        ''' get tags listing '''
        all_items = []
        for item in self.widgetshelper.kodidb.files("videodb://movies/tags"):
    ...
    def refplaylist(self):
        ...
        ref_items = self.widgetshelper.kodidb.movies(filters=playlist_filter)
        # get all items not in playlist
        not_playlist_filter = [{"operator": "isnot", "field": "playlist", "value": tag_label}]
        all_items = self.widgetshelper.kodidb.movies(filters=not_playlist_filter)
        # return list sorted by recommended score
    ...
    def recommended(self):
        ...
        return self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_RATING, filters=filters,
    ...
    def recent(self):
        ...
        if self.options.get("tag"):
            filters.append({"operator": "contains", "field": "tag", "value": self.options["tag"]})
        return self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_DATEADDED, filters=filters,
                                                limits=(0, self.options["limit"]))
    ...
    def random(self):
        ...
        if self.options.get("tag"):
            filters.append({"operator": "contains", "field": "tag", "value": self.options["tag"]})
        return self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_RANDOM, filters=filters,
                                                limits=(0, self.options["limit"]))
    ...
    def inprogress(self):
        ...
        if self.options.get("tag"):
            filters.append({"operator": "contains", "field": "tag", "value": self.options["tag"]})
        return self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_LASTPLAYED,
                                                filters=filters,
                                                limits=(0, self.options["limit"]))
    ...
    def unwatched(self):
        ...
        if self.options.get("tag"):
            filters.append({"operator": "contains", "field": "tag", "value": self.options["tag"]})
        return self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_TITLE, filters=filters,
                                                limits=(0, self.options["limit"]))
    ...
    def similar(self):
        ...
        if imdb_id:
            # get movie by imdbid
            ref_movie = self.widgetshelper.kodidb.movie_by_imdbid(imdb_id)
        if not ref_movie:
    ...
    def forgenre(self):
        ...
        if not genre:
            genre = self.widgetshelper.kodidb.genres("movie")[0]["label"]
        all_items = []
        if genre:
    ...
    def browsegenres(self):
        '''
            ...
            TODO: get auto generated collage pictures from widgetshelper ?
        '''
        all_genres = self.widgetshelper.kodidb.genres("movie")
        return self.widgetshelper.process_method_on_list(self.get_genre_artwork, all_genres)
    ...
    def get_genre_artwork(self, genre_json):
        ...
        if self.options.get("tag"):
            genre_json["file"] = "plugin://script.pirate.widgets?mediatype=movies&action=forgenre&tag=%s&genre=%s"\
    ...
    def get_random_watched_movie(self):
        ...
        movies = self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_RANDOM,
    ...
    def get_genre_movies(self, genre, hide_watched=False, limit=100, sort=kodi_constants.SORT_RANDOM):
        ...
        if hide_watched:
            filters.append(kodi_constants.FILTER_UNWATCHED)
        return self.widgetshelper.kodidb.movies(sort=sort, filters=filters, limits=(0, limit))

    def sort_by_recommended(self, all_items, ref_movies=None):
        ''' sort list of movies by recommended score '''
        if not ref_movies:
            # get recently watched movies
            ref_movies = self.widgetshelper.kodidb.movies(sort=kodi_constants.SORT_LASTPLAYED,
                                                          filters=[kodi_constants.FILTER_WATCHED],
